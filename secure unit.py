...
# Unit tests
class TestSecureCryptoWallet(unittest.TestCase):
    def setUp(self):
        self.app = SecureCryptoWalletApp(Mock())
        self.app.blockchain = Blockchain()  # Replace with any needed setup

    def test_create_transaction(self):
        self.app.username_entry.insert(0, "sender_name")
        self.app.recipient_entry.insert(0, "recipient_name")
        self.app.amount_entry.insert(0, "100")
        initial_transactions = len(self.app.blockchain.current_transactions)
        self.app.create_transaction()
        new_transactions = len(self.app.blockchain.current_transactions)
        self.assertEqual(new_transactions, initial_transactions + 1)

    def test_mine_block(self):
        initial_chain_length = len(self.app.blockchain.chain)
        self.app.mine_block()
        new_chain_length = len(self.app.blockchain.chain)
        self.assertEqual(new_chain_length, initial_chain_length + 1)

    def test_transaction_encryption_and_decryption(self):
        sender = "Alice"
        recipient = "Bob"
        amount = 50.0
        encryption_key = get_random_bytes(16)

        encrypted_transaction = self.app.blockchain.encrypt_transaction(
            sender, recipient, amount, encryption_key
        )

        decrypted_transaction = self.app.blockchain.decrypt_transaction(
            encrypted_transaction, encryption_key
        )

        self.assertEqual(decrypted_transaction['sender'], sender)
        self.assertEqual(decrypted_transaction['recipient'], recipient)
        self.assertEqual(decrypted_transaction['amount'], amount)
